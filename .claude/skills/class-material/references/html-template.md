...
上位階層へ戻るリンクはここに書かない。リンク先は公開サイトで年度と教科名が別階層に分かれて初めて存在するため、`scripts/build_index.py` が Pages デプロイ時に `<header class="doc-head">` の直前へ挿入する。この行を書かないことが、ローカルで開いたときに行き先の無いリンクを出さない条件になる。
...
- 説明文の書き方は `writing-style-guide.md` § 8 を参照する
